...
| 字段名 | 类型 | 可空 | 默认值 | 约束 | 说明 |
|--------|------|------|--------|------|------|
| `id` | `character varying(255)` | NO |  | PK | OIDC设备码的唯一标识 |
...
| `user_id` | `text` | YES |  | FK → users.id | 关联的用户ID |
| `client_id` | `character varying(255)` | NO |  |  | 关联的OIDC客户端ID |
| `grant_id` | `character varying(255)` | YES |  |  | 关联的授权记录ID |
| `user_code` | `character varying(255)` | YES |  |  | 用于设备授权流程的用户输入代码 |
