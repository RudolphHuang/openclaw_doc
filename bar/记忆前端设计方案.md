...
### 4.3 调用 OpenAI API（使用 Function Calling）

```typescript
// src/services/chat/openai.ts

import OpenAI from 'openai';
import { agentMemoryTools } from '@/tools/agentMemory/definition';
import { handleAgentMemoryTool } from '@/tools/agentMemory/handler';
...
/**
 * 调用 OpenAI Chat Completions API，支持 Function Calling
 * 
 * 流程：
 * 1. 第一次调用：发送 systemPrompt + messages + tools
 * 2. 如果返回 function_call，处理工具调用
 * 3. 第二次调用：将工具结果加入 messages，再次调用
 * 4. 返回最终回复
 */
export async function* streamChatWithMemory(
  params: ChatParams
): AsyncGenerator<StreamChunk> {
  const { messages, memoryReadEnabled, memoryWriteEnabled } = params;
  ...
  // 4. 确定可用工具
  const tools = (memoryReadEnabled || memoryWriteEnabled) 
    ? agentMemoryTools 
    : undefined;
  ...
  if (toolCalls.length > 0) {
    ...
    for (const toolCall of toolCalls) {
      ...
      // 显示工具调用状态
      yield { type: 'tool_call_start', toolName: name };
      ...
      if (name.startsWith('agent_memory_')) {
        ...
        if (['agent_memory_create', 'agent_memory_update', 'agent_memory_delete'].includes(name)) {
          if (!memoryWriteEnabled) {
            ...
            result = await handleAgentMemoryTool(name, JSON.parse(args));
          }
        } else {
          result = await handleAgentMemoryTool(name, JSON.parse(args));
        }
        ...
        // 处理其他工具...
        result = await handleOtherTool(name, JSON.parse(args));
      }

      yield { type: 'tool_call_end', toolName: name, result };
...
async function handleUserMessage(userMessage: string) {
  ...
  // 开始流式输出
  const stream = streamChatWithMemory({
    messages,
  ...
  for await (const chunk of stream) {
    switch (chunk.type) {
      ...
      case 'tool_call_start':
        // 显示工具调用开始
        showToolStatus(`正在${getToolName(chunk.toolName)}...`);
        break;
      
      case 'tool_call_end':
        // 显示工具调用完成
        if (chunk.result.type === 'success') {
          showToolStatus(`${getToolName(chunk.toolName)}完成 ✓`);
        } else {
          showToolStatus(`${getToolName(chunk.toolName)}失败 ✗`, 'error');
        }
        break;
    }
  }
...
function getToolName(name: string): string {
  const map: Record<string, string> = {
    'agent_memory_read': '读取记忆',
    'agent_memory_create': '创建记忆',
    'agent_memory_update': '更新记忆',
    'agent_memory_delete': '删除记忆',
  };
  ...
  "choices": [{
    "delta": {
      "tool_calls": [{
        ...
        "function": {
          "name": "agent_memory_create",
          "arguments": "{\"category\":\"feedback\",\"title\":\"用户偏好简洁回答\",\"content\":\"用户喜欢简洁的回答\\n\\n**Why:** 避免信息过载\\n\\n**How to apply:** 先给出简洁答案\"}"
        }
      }]
    },
...

{
  ...
  "messages": [
    ...
    },
    {
      ...
      "content": "Memory created: 用户偏好简洁回答 (agent_memory_01HQxxx)"
    }
  ],
  "stream": true
}
