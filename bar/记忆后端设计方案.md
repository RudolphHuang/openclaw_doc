...
│                    Agent Memory 系统                         │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐  │
│  │   tRPC API   │───▶│  Model 层    │───▶│  Database    │  │
│  │              │    │              │    │              │  │
│  │ - agentMemory│    │ - AgentMemory│    │ - agent_memories│ │
│  │ - chatAgent  │    │ - AgentMemory│    │ - agent_memory_│  │
│  │   Memory     │    │   Index      │    │   index      │  │
│  └──────────────┘    └──────────────┘    └──────────────┘  │
└─────────────────────────────────────────────────────────────┘
                              │
...
**文件**: `src/server/services/agentMemory/contextProvider.ts`
...
export async function buildMemoryPrompt(
  ...
  const indexModel = new AgentMemoryIndexModel(db, userId);
  const index = await indexModel.queryByAgentId(DEFAULT_AGENT_ID);
  ...
  lines.push(
    ...
    '- agent_memory_read(entryId) - Load memory details',
    '- agent_memory_create(category, title, content) - Create new memory',
    '- agent_memory_update(entryId, content) - Update memory',
...
### 5.2 集成到 Chat Service

在聊天服务中注入记忆上下文：

```typescript
// src/server/services/chat/index.ts
import { buildMemoryPrompt } from '../agentMemory/contextProvider';

export async function createChatCompletion(
  params: ChatCompletionParams,
) {
  const { db, userId, messages } = params;

  // 1. 构建记忆提示词
  const memoryPrompt = await buildMemoryPrompt(db, userId);
  ...
  return callLLM({
    systemPrompt,
    messages,
    tools: [...baseTools, ...agentMemoryTools], // 注入记忆工具
  });
}
```
